...
from util import my_api_key, get_related_strings, write_jsonl, wait_until_kam
...
# file_path = "/home/siyue001/Projects/llm2vec_reason_dream/TheoremAug/gen_problem_solution/deepseek/solution_check/before_hard_negative_tmp.pkl"
file_path = "/home/siyue001/Projects/llm2vec_reason_dream/TheoremAug/gen_problem_solution/before_hard_negative_tmp.pkl"
...
for instance, pairs in pairs_by_instance.items():
    ...
    for query_idx, pos_idx in pair_combinations:
        ...
        # print(f'\n\nquery: \n{query_problem} \n\npos: \n{p} \n\nneg: \n{n}\n{i}\n~~~~~~~')
    #     print(query_idx)
    #     if instance=="Chebyshev's Inequality":
    #         print(neg_instance)
    #         print(neg[:150],'\n')
    # if instance=="Chebyshev's Inequality":
    #     assert 1==2
    ...
    # p2ps
    if domain not in ['finance formula', 'data structure']:
        pair_combinations = [(i, (i + 1) % len(problems)) for i in range(len(problems))]
        pair_combinations = pair_combinations[:min(len(pair_combinations), 5)]
...
file_path = 'TheoremAug_tmp_test.jsonl'
write_jsonl(file_path, finals)



# output_path = Path('gen_hard_negative_output.jsonl')
...
#             related = [instance] + get_related_strings(similar_question_mapping, instance)
...
# The hard negative document is negative because it uses a {domain} that cannot be used to solve the problem in the user query.
# The hard negative document should not be related to {related}.
...
#                     max_tokens=4000,
#                     stream=False,
#                     temperature=1,  # https://api-docs.deepseek.com/quick_start/parameter_settings
#                 )
...
#                 print(f"Processed request {custom_id}")
                
#             except Exception as e:
#                 print(f"Error processing request {custom_id}: {str(e)}")
#                 continue
