...
def main():
    ...
    # Add these debug prints after model initialization
    logger.info(f"Initializing model from: {model_args.model_name_or_path}")
    logger.info(f"Model type: {type(model)}")
    logger.info(f"Base model type: {type(model.model)}")
    logger.info(f"Model config type: {type(model.model.config)}\n")
